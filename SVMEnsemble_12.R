...
SVMEnsemble_12_howtopredict<-function(SELECTION,test_normal,test_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22,d23,created23,model23,svm23,d31,created31,model31,svm31,d32,created32,model32,svm32,d33,created33,model33,svm33,d41,created41,model41,svm41,d42,created42,model42,svm42,d43,created43,model43,svm43){
...
pred43<-0
radius43<-0
bsc43<-0 #block spearman correlation
if(created43){
	pred43 = predict(svm43, test_normal[test_index,])
	pred43 = as.numeric(pred43)
	tmp43<-rbind(d43,c(pred43,tpc1))
	tmp43<-as.data.frame(tmp43)
	dist43<-dist(tmp43, method = "euclidean")
	dist43<-as.matrix(dist43)
	tmp431<-t(rbind(model43[,-ncol(model43)],test_normal[test_index,-ncol(test_normal)])) #transpose
	cor43<-cor(tmp431, method = c('spearman'))
	bsc43<-sum(cor43[1:nrow(cor43)-1,ncol(cor43)])/nrow(d43)
}
mc43<-0
if(created43) {
	found<-which(dist43[1:nrow(dist43)-1,ncol(dist43)]<=RAD)
	neighbors43<-rownames(dist43)[found]
	nbr43<-length(found)
	if(nbr43>0){
		corIndx<-which(rownames(cor43) %in% neighbors43)   #we will look at the last column of the matrix. So matching by rownames
		mc43<-mean(cor43[corIndx,ncol(cor43)])
		if(DEBUG){
			print("svm43")
			print(neighbors43)
			print(rownames(cor43)[corIndx])
		}
	} else {
		mc43<-0
	}
...
if(DEBUG){
	...
	if(!is.null(tmp43)){
		df43<-tmp43
		df43$label<-'old'
		df43$label[nrow(tmp43)]<-'new'
		df43<-as.data.frame(df43)
		p43<-ggplot(df43, aes(auc,pc1, color=factor(label))) + geom_point() + theme_classic()
	} else p43<-ggplot(blank, aes(x,y)) + theme_classic()
	png(filename = paste(path,"SVE_12_DEBUG_testIndex",test_index,".png",sep=""), width = 10, height = 10, units = "in", res = 200)
	plot_grid(p11,p12,p13,p21,p22,p23,p31,p32,p33,p41,p42,p43,ncol = 3, labels = "AUTO", label_size = 16)
	dev.off()
}
MC<-c(mc11,mc12,mc13,mc21,mc22,mc23,mc31,mc32,mc33,mc41,mc42,mc43)
NBR<-c(nbr11,nbr12,nbr13,nbr21,nbr22,nbr23,nbr31,nbr32,nbr33,nbr41,nbr42,nbr43)
PRED<-c(pred11,pred12,pred13,pred21,pred22,pred23,pred31,pred32,pred33,pred41,pred42,pred43)
BSC<-c(bsc11,bsc12,bsc13,bsc21,bsc22,bsc23,bsc31,bsc32,bsc33,bsc41,bsc42,bsc43)
if(DEBUG) {
	...
	print(paste('pred43:',pred43,' nbr43:',nbr43,' mc43:',mc43,' bsc43:',bsc43,sep=''))
}
sumNbr<-sum(NBR)
if(SELECTION==0){
	...
	r<-2
	while(sumNbr==0 && r<=12){   ##the case where such a radius is chosen that all meanCorrelation values is 0
	radius<-RADs[r]
	r<-r+1
	if(DEBUG) print(radius)
	RAD<-radius
	...
	if(DEBUG) {
		...
		print(paste('pred43:',pred43,' nbr43:',nbr43,' mc43:',mc43,sep=''))
	}
...
prd
} #end SVMEnsemble_12_howtopredict()




SVMEnsemble_12<-function(blocks,percTesting,SELECTION,fold,N,CV,DEBUG,ALLDATA,DIST,y,yName,PATH){
...
if(length(CL43)>0){
	m43<-TRUE
	d43<-DIST[(which(rownames(DIST) %in% CL43)),]
	d43<-as.data.frame(d43)
	tmp<-which(rownames(ALLDATA) %in% CL43)
	model43<-ALLDATA[tmp,]
	model43<-as.data.frame(model43)
}
...
ix43=-1
if(m43) {
	ix43<-which(rownames(model43) %in% trainNames)
	if(length(ix43)==0) ix43=-1
}
sigma43=-1
C43=-1
...
created43<-FALSE #model created or not
if(m43){
	if(length(ix43)>MINSAMPLES){
		created43<-TRUE
		train43 = model43[ix43, ]
		svm43 <- train(as.formula(formula), data = train43, method = "svmRadial", tuneLength = 10)
		sigma43<-svm43$bestTune$sigma
		C43<-svm43$bestTune$C
	}
}
...
for(train_index in 1:L){
	...
	predTrain<-c(predTrain,SVMEnsemble_12_howtopredict(SELECTION,train_normal,train_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22,d23,created23,model23,svm23,d31,created31,model31,svm31,d32,created32,model32,svm32,d33,created33,model33,svm33,d41,created41,model41,svm41,d42,created42,model42,svm42,d43,created43,model43,svm43))
...
for(test_index in 1:L){
	...
	predTest<-c(predTest,SVMEnsemble_12_howtopredict(SELECTION,test_normal,test_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22,d23,created23,model23,svm23,d31,created31,model31,svm31,d32,created32,model32,svm32,d33,created33,model33,svm33,d41,created41,model41,svm41,d42,created42,model42,svm42,d43,created43,model43,svm43))
...
write.csv(tbw,paste(PATH,'SVMEnsemble_12_',drug_names[drug_index],'.csv',sep = ''), row.names = F, quote = F)
...
print(paste('k=12_CV_RMSE',sqrt(cv_sse/nrow(ALLDATA)),sep=' '))

list(c(meanSqError,bestMSE),c(bSigma11,bC11,bSigma12,bC12,bSigma13,bC13,bSigma21,bC21,bSigma22,bC22,bSigma23,bC23,bSigma31,bC31,bSigma32,bC32,bSigma33,bC33,bSigma41,bC41,bSigma42,bC42,bSigma43,bC43),trNames,teNames)

} #end SVMEnsemble_12()



SVMEnsemble_12_Replicate<-function(params,blocks,SELECTION,DEBUG,ALLDATA,DIST,y,yName){
...
sigma43<-tuningParameters[23]
C43<-tuningParameters[24]
...
if(m43){
	if(length(ix43)>MINSAMPLES){
		created43<-TRUE
		train43 = model43[ix43, ]
		svm43 <- train(as.formula(formula), data = train43, method = "svmRadial", tuneGrid = expand.grid(sigma=sigma43, C=C43))
	}
}
...
SVMEnsemble_12_Predict<-function(params,blocks,TEST,SELECTION,DEBUG,ALLDATA,DIST,y,yName){
