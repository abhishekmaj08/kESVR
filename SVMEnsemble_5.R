...
SVMEnsemble_5_howtopredict<-function(SELECTION,test_normal,test_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22){
...
RADs<-c(abs(max(d11$auc)-min(d11$auc))/2,abs(max(d12$auc)-min(d12$auc))/2,abs(max(d13$auc)-min(d13$auc))/2,abs(max(d21$auc)-min(d21$auc))/2,abs(max(d22$auc)-min(d22$auc))/2)
if(DEBUG) print(paste('rad11','rad12','rad13','rad21','rad22',sep=' '))
if(DEBUG) print(RADs)
RADs<-sort(RADs)
radius<-RADs[1]
...
if(DEBUG){
	...
	png(filename = paste(path,"SVE_5_DEBUG_testIndex",test_index,".png",sep=""), width = 10, height = 10, units = "in", res = 200)
	plot_grid(p11,p12,p13,p21,p22,ncol = 3, labels = "AUTO", label_size = 16)
	dev.off()
}
MC<-c(mc11,mc12,mc13,mc21,mc22)
NBR<-c(nbr11,nbr12,nbr13,nbr21,nbr22)
PRED<-c(pred11,pred12,pred13,pred21,pred22)
BSC<-c(bsc11,bsc12,bsc13,bsc21,bsc22)
if(DEBUG) {
...
if(SELECTION==0){
	...
	r<-2
	while(sumNbr==0 && r<=5){   ##the case where such a radius is chosen that all meanCorrelation values is 0
	radius<-RADs[r]
	r<-r+1
	if(DEBUG) print(radius)
	RAD<-radius
...
prd
} #end SVMEnsemble_5_howtopredict()




SVMEnsemble_5<-function(blocks,percTesting,SELECTION,fold,N,CV,DEBUG,ALLDATA,DIST,y,yName,PATH){
...
for(train_index in 1:L){
	...
	predTrain<-c(predTrain,SVMEnsemble_5_howtopredict(SELECTION,train_normal,train_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22))
...
for(test_index in 1:L){
	...
	predTest<-c(predTest,SVMEnsemble_5_howtopredict(SELECTION,test_normal,test_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22))
...
write.csv(tbw,paste(PATH,'SVMEnsemble_5_',drug_names[drug_index],'.csv',sep = ''), row.names = F, quote = F)
...
print(paste('k=5_CV_RMSE',sqrt(cv_sse/nrow(ALLDATA)),sep=' '))

list(c(meanSqError,bestMSE),c(bSigma11,bC11,bSigma12,bC12,bSigma13,bC13,bSigma21,bC21,bSigma22,bC22),trNames,teNames)

} #end SVMEnsemble_5()



SVMEnsemble_5_Replicate<-function(params,blocks,SELECTION,DEBUG,ALLDATA,DIST,y,yName){
...
SVMEnsemble_5_Predict<-function(params,blocks,TEST,SELECTION,DEBUG,ALLDATA,DIST,y,yName){
