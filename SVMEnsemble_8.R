...
SVMEnsemble_8_howtopredict<-function(SELECTION,test_normal,test_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22,d23,created23,model23,svm23,d31,created31,model31,svm31,d32,created32,model32,svm32){
...
RADs<-c(abs(max(d11$auc)-min(d11$auc))/2,abs(max(d12$auc)-min(d12$auc))/2,abs(max(d13$auc)-min(d13$auc))/2,abs(max(d21$auc)-min(d21$auc))/2,abs(max(d22$auc)-min(d22$auc))/2,abs(max(d23$auc)-min(d23$auc))/2,abs(max(d31$auc)-min(d31$auc))/2,abs(max(d32$auc)-min(d32$auc))/2)
if(DEBUG) print(paste('rad11','rad12','rad13','rad21','rad22','rad23','rad31','rad32',sep=' '))
if(DEBUG) print(RADs)
RADs<-sort(RADs)
radius<-RADs[1]
...
if(DEBUG){
	...
	png(filename = paste(path,"SVE_8_DEBUG_testIndex",test_index,".png",sep=""), width = 10, height = 10, units = "in", res = 200)
	plot_grid(p11,p12,p13,p21,p22,p23,p31,p32,ncol = 3, labels = "AUTO", label_size = 16)
	dev.off()
}
MC<-c(mc11,mc12,mc13,mc21,mc22,mc23,mc31,mc32)
NBR<-c(nbr11,nbr12,nbr13,nbr21,nbr22,nbr23,nbr31,nbr32)
PRED<-c(pred11,pred12,pred13,pred21,pred22,pred23,pred31,pred32)
BSC<-c(bsc11,bsc12,bsc13,bsc21,bsc22,bsc23,bsc31,bsc32)
if(DEBUG) {
...
if(SELECTION==0){
	...
	r<-2
	while(sumNbr==0 && r<=8){   ##the case where such a radius is chosen that all meanCorrelation values is 0
	radius<-RADs[r]
	r<-r+1
	if(DEBUG) print(radius)
	RAD<-radius
...
prd
} #end SVMEnsemble_8_howtopredict()




SVMEnsemble_8<-function(blocks,percTesting,SELECTION,fold,N,CV,DEBUG,ALLDATA,DIST,y,yName,PATH){
...
for(train_index in 1:L){
	...
	predTrain<-c(predTrain,SVMEnsemble_8_howtopredict(SELECTION,train_normal,train_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22,d23,created23,model23,svm23,d31,created31,model31,svm31,d32,created32,model32,svm32))
...
for(test_index in 1:L){
	...
	predTest<-c(predTest,SVMEnsemble_8_howtopredict(SELECTION,test_normal,test_index,y,yName,d11,created11,model11,svm11,d12,created12,model12,svm12,d13,created13,model13,svm13,d21,created21,model21,svm21,d22,created22,model22,svm22,d23,created23,model23,svm23,d31,created31,model31,svm31,d32,created32,model32,svm32))
...
write.csv(tbw,paste(PATH,'SVMEnsemble_8_',drug_names[drug_index],'.csv',sep = ''), row.names = F, quote = F)
...
print(paste('k=8_CV_RMSE',sqrt(cv_sse/nrow(ALLDATA)),sep=' '))

list(c(meanSqError,bestMSE),c(bSigma11,bC11,bSigma12,bC12,bSigma13,bC13,bSigma21,bC21,bSigma22,bC22,bSigma23,bC23,bSigma31,bC31,bSigma32,bC32),trNames,teNames)

} #end SVMEnsemble_8()



SVMEnsemble_8_Replicate<-function(params,blocks,SELECTION,DEBUG,ALLDATA,DIST,y,yName){
...
SVMEnsemble_8_Predict<-function(params,blocks,TEST,SELECTION,DEBUG,ALLDATA,DIST,y,yName){
